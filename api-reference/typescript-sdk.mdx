---
title: 'Ragen AI TypeScript SDK Reference'
sidebarTitle: 'TypeScript SDK'
description: 'Complete reference for @webamigos/ragen-sdk-ts: typed API client with streaming, auto-retry, file upload helpers, and edge runtime support.'
---
...
Pass a configuration object when you construct the client. Only `apiKey` and `baseURL` are required — everything else has a sensible default.
...
| `baseURL`     | `string`       | `process.env.RAGEN_BASE_URL` | Base URL of your Ragen instance, e.g. `http://localhost:3001/v1`.     |
