...
<Accordion title="ragen_chat — Send a message to an assistant">
  ...
  **Response**
  ...
  ```json
  {
    "success": false,
    "status": 403,
    "error": "This API key is scoped to a different assistant than 'asst-abc123'"
  }
  ```

  A `403` means the key and the request disagree — you named an assistant the key is not scoped to, or named one at all on a knowledge-base key. A `404` is the other case, an assistant that does not exist for this organization:
...
<Accordion title="ragen_list_assistants — List available assistants">
  List the assistants your API key can reach. A knowledge-base key sees every assistant its organization owns; a key created for one assistant sees that one. Use this to discover assistant IDs before calling `ragen_chat` — or, if exactly one comes back, to confirm you can omit the parameter.
  ...
  **Response**
  ...
  Scoping is automatic and follows the key: the organization's assistants for a knowledge-base key, the single bound one for an assistant-scoped key. There is no separate access configuration required.
</Accordion>
