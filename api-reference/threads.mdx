...
<ParamField body="assistant_id" type="string">
  **Ragen extension.** Bind the thread to a specific assistant using its `asst-<projectId>` ID. Optional, and it must agree with the key's scope: a key bound to one assistant refuses any other with `403`, and a knowledge-base key refuses the field entirely and opens the thread against the knowledge base. Omit it and the key decides. See [Authentication](/api-reference/authentication#what-a-key-can-reach).
</ParamField>
...
If the organization has **Delete threads** turned off, the request is refused with `403 Forbidden` and nothing is deleted:

```json
{
  "statusCode": 403,
  "message": "This organization cannot delete threads",
  "error": "Forbidden"
}
```

The same setting refuses `DELETE /v1/threads/{id}/messages/{message_id}` with the same response, because deleting every message empties a thread just as well. A platform administrator controls the setting in the admin panel — see [Organization features](/security/admin-panel#organization-features).
