---
title: 'Chat Completions — OpenAI-Compatible RAG Endpoint'
sidebarTitle: 'Chat Completions'
description: 'POST /v1/chat/completions — OpenAI-compatible endpoint that runs the full Ragen RAG pipeline with multi-turn support.'
---
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X POST $RAGEN_BASE_URL/v1/chat/completions \
...
Chat Completions run a full RAG pipeline (vector search + rerank + LLM call), placing them on the expensive tier:
