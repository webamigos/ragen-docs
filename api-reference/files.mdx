...
sidebarTitle: 'Files'
description: 'POST, GET, DELETE /v1/files — Upload documents to a Ragen project, poll for processing status, and manage your knowledge base.'
---
...
`DELETE /v1/files/{id}` — removes the file completely: the database row, the storage object, any thumbnail, its `UserDocument` record, and all embeddings from the vector store. External cleanup (storage and vector store) is best-effort — a transient blip won't block the database delete.
...
<Tabs>
  <Tab title="TypeScript SDK">
    ...
    // Upload and wait in one call
    async function uploadAndWait(filePath: string) {
      const uploaded = await ragen.files.create({
        file: createReadStream(filePath),
        purpose: 'knowledge_base',
      });
      return waitUntilProcessed(uploaded.id);
    }
    ```
  </Tab>
  ...
  <Tab title="curl">
    ```bash
    # Upload
    curl -X POST $RAGEN_BASE_URL/v1/files \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -F "file=@handbook.pdf" \
      -F "purpose=knowledge_base"

    # Poll status
    curl $RAGEN_BASE_URL/v1/files/file-abc123 \
      -H "Authorization: Bearer YOUR_API_KEY"

    # List files
    curl "$RAGEN_BASE_URL/v1/files?limit=20" \
      -H "Authorization: Bearer YOUR_API_KEY"

    # Delete
    curl -X DELETE $RAGEN_BASE_URL/v1/files/file-abc123 \
      -H "Authorization: Bearer YOUR_API_KEY"
    ```
  </Tab>
</Tabs>
...
| Endpoint | Limit |
|---|---|
| `POST /v1/files` | 10 requests / minute (expensive tier) |
| `GET /v1/files`, retrieve, delete | 20 requests / minute (default tier) |
