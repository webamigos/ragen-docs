...
The Files API is an OpenAI-compatible interface for adding documents to your Ragen knowledge base and managing them over time. Files are scoped to the API key, the same way its answers are: a key bound to **one assistant** lists and uploads that assistant's files, and a key scoped to **the knowledge base** works with the files that belong to no assistant. Either way a file becomes available to the [Chat Completions](/api-reference/chat-completions) endpoint once processing completes. See [Authentication](/api-reference/authentication#what-a-key-can-reach).
...
`GET /v1/files` — returns a paginated list of the files the key can reach.
...
`GET /v1/files/{id}` — returns the full OpenAI `file` object for a single file, or `404` if the file does not exist or falls outside what the key can reach.
...
| `404` | `not_found_error` | File doesn't exist, or is outside what the key can reach |
