---
title: 'Chat API — Single-Turn RAG Question and Answer Endpoint'
sidebarTitle: 'Chat'
description: 'POST /v1/chat — Send a single message to a Ragen assistant and get a RAG-grounded answer back. Supports streaming via Server-Sent Events.'
---
...
<ParamField body="assistant_id" type="string">
  The assistant (project) ID to query. Optional — the API key already carries a scope, and this field must agree with it. Find an ID in the dashboard URL when viewing a project (`…/projects/<assistant_id>`), via `GET /v1/assistants`, or under **Settings → Assistant settings**.
</ParamField>
...
<Steps>
  ...
  <Step title="Assistant resolution">
    The assistant comes from the API key's scope. A key bound to one assistant resolves to it; a knowledge-base key resolves to no assistant, and retrieval runs against the documents that belong to none. An `assistant_id` in the request must agree with that, or the call is refused with `403`.
  </Step>
