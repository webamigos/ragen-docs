...
<Tabs>
  <Tab title="Storage">
    ...
    <Tabs>
      ...
      <Tab title="s3">
        Any S3-compatible object store. Leave `S3_ENDPOINT_URL` unset for real AWS S3, or set it to point at Cloudflare R2, Scaleway, MinIO, Ceph, or the [bundled RustFS](/configuration/storage#bundled-object-storage-rustfs).
  ...
  <Tab title="Encryption">
    ...
    <Warning>
      Encryption is opt-in and off by default in local development. A process needs an encryption provider unless it is plainly not a deployment: `NODE_ENV=development` or `test`, or `TARGET_ENV` set to `local`, `test`, `e2e` or `ci`. Anywhere else — **including a build that simply never set `TARGET_ENV`** — a missing provider stops it, unless you set `ALLOW_UNENCRYPTED=1` — which is logged as a critical security event. Set a provider before going live. See [Encryption](/security/encryption).
    </Warning>
...
Database, vector store and Redis are listed
...
| Variable | Required | Default | Description |
|---|---|---|---|
| `CONNECTOR_TRUSTED_HOSTS` | Optional | — | Comma-separated hosts your deployment vouches for. A catalogue connector pointing at one connects without the private-address policy. Set it on the web app, the API and the admin panel. See [A host your deployment vouches for](/integrations/adding-a-connector#a-host-your-deployment-vouches-for). |
