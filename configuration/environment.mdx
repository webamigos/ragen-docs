...
| `LLM_GATEWAY` | Optional | `litellm` | `litellm` sends model calls to a proxy you run; `native` has Ragen call providers itself ([Model gateway](/configuration/model-gateway)) |
| `LLM_ROUTES_PATH` | Optional | — | Route table to use instead of the shipped one, under `native` |
| `LITELLM_PROXY_URL` | **Required** | `http://localhost:4000` | URL of the LiteLLM proxy; where model calls go on the proxy path |
...
| `REDIS_URL` | Optional* | `redis://localhost:56379` | Redis connection URL (*required by the background worker; optional for the web app) |
