...
The field is off by default, because an endpoint that does not understand `response_format: { type: "json_schema" }` may reject the call. With it off, the schema is dropped and the model is asked for plain JSON, so it has to guess the shape. That fails without an obvious error: before the shipped Scaleway route had this line, Mistral answered with its own field names, and every Brain extraction failed validation.
