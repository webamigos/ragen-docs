...
All three application services send model requests to a single LiteLLM proxy. Changing which model servers that proxy points to requires only an edit to `infra/litellm/config.yaml` and the matching environment variables — no application code changes.
...
Register the model in `infra/litellm/config.yaml`. The `hosted_vllm/` prefix is LiteLLM's route for a self-hosted OpenAI-compatible vLLM server:

```yaml title="infra/litellm/config.yaml"
model_list:
...
Register it in `infra/litellm/config.yaml`. Use `ollama_chat/` rather than `ollama/` — the chat endpoint produces better results:
