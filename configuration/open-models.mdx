...
<Accordion title="Structured output (multi-query expansion)">
  Multi-query expansion asks the model to return a JSON object that matches a schema, and so do Brain extraction, RAG scoring and the guardrail policy judge. **Ragen sends the schema only when you set `structuredOutputs: true` on the route.** Without the flag, Ragen asks the model for plain JSON, and the model has to guess the shape. See [Structured outputs](/configuration/model-gateway#structured-outputs).
