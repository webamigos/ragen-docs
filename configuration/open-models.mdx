...
When you point every model role at a local endpoint, nothing your users type and none of your document content leaves your network. Nearly every model call Ragen makes goes through the LiteLLM proxy you run — swap that proxy's targets to local servers and the work that touches your data happens entirely on machines you control. "Fully offline" is a configuration you arrive at intentionally, not something you inherit: you must set all four model roles, disable outbound reranking, set `DOCLING_STRICT=1`, and check the [what still reaches outward](#what-still-reaches-outward) table below.

## How the Routing Works

All application services send model requests to a single LiteLLM proxy. Changing which model servers that proxy points to requires only an edit to the LiteLLM configuration file (`litellm/config.yaml`) and the matching environment variables — no application code changes.

```
Web app   ─┐
API       ├─→  LiteLLM proxy  ─→  vLLM / Ollama on your GPU box
Worker   ─┘         ↑
               litellm/config.yaml
```
...
Both vLLM and Ollama expose an OpenAI-compatible API, which is all LiteLLM needs to route requests to them.
...
Start the vLLM server with a served model name that matches what you will put in LiteLLM's config:
...
Register the model in your LiteLLM configuration file. The `hosted_vllm/` prefix is LiteLLM's route for a self-hosted OpenAI-compatible vLLM server:
...
If vLLM runs on a separate GPU host, replace `http://vllm:8000` with that host's address — LiteLLM only needs network access to it.

After editing the config, restart the proxy and confirm it sees the model:
...
Register it in your LiteLLM configuration file. Use `ollama_chat/` rather than `ollama/` — the chat endpoint produces better results:
...
Both vLLM and Ollama expose a `/v1/embeddings` endpoint. Register an embedding model separately from the chat model so you can hide it from the chat picker:

```yaml title="litellm/config.yaml"
model_list:
  - model_name: local-embed
    litellm_params:
      model: openai/local-embed       # vLLM path
...
|---|---|---|
| Content moderation | Off (`MODERATION_ENABLED` unset) | Leave it off. When enabled, it calls the OpenAI Moderation API **directly** — it is the one model call that does not go through LiteLLM and cannot be redirected. |
...
| Reranking | Off (`FEATURE_FLAG_RERANKING` unset) | Leave it off, or use `RERANK_PROVIDER=scaleway` pointed at your own vLLM rerank server. `RERANK_PROVIDER=cohere` routes through AWS Bedrock by definition. |
...
| Speech (TTS/STT) | Off unless configured | `SPEECH_PROVIDER=elevenlabs` leaves the network. The OpenAI speech path routes through LiteLLM when `LITELLM_PROXY_URL` is set, so it can be served locally. |
...
| LiteLLM → Langfuse tracing | Off unless `LANGFUSE_*` vars are set | Leave those variables unset, or point them at a self-hosted Langfuse instance. |
...
```bash
# The proxy serves your local model IDs
curl http://localhost:4000/v1/models
