...
Every model call Ragen makes — answering questions, rephrasing queries, summarising documents, and generating embeddings — routes through a single LiteLLM proxy. You set `LITELLM_PROXY_URL` to tell Ragen where that proxy lives, and you configure the models themselves in `infra/litellm/config.yaml`. The environment variables below tell each part of the application which model name to request from the proxy.
...
The LiteLLM proxy supports every major LLM provider. Add the models you want to use to `infra/litellm/config.yaml`, then set the corresponding API key environment variable.
...
Each model you want to use must appear in `infra/litellm/config.yaml`. The `model_name` field is the identifier you set in `DEFAULT_MODEL`, `REPHRASE_MODEL`, etc.
