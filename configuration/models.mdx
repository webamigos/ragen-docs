...
| `OPENAI_API_KEY` | — | Also used by the `content-moderation` [guardrail rule](/security/guardrails), which calls the OpenAI Moderation API directly rather than through the route table |
| `OPENAI_MODERATION_KEY` | — | Dedicated key for that rule; falls back to `OPENAI_API_KEY` |
