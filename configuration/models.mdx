...
sidebarTitle: 'Models'
description: 'Route all LLM calls through a LiteLLM proxy to any provider — OpenAI, Anthropic, Google, Bedrock, Scaleway, or a local vLLM/Ollama server.'
---

Every model call Ragen makes — answering questions, rephrasing queries, summarising documents, and generating embeddings — routes through a single LiteLLM proxy. You set `LITELLM_PROXY_URL` to tell Ragen where that proxy lives, and you configure the available models in the LiteLLM configuration file (`litellm/config.yaml` in your deployment). The environment variables below tell each part of the stack which model name to request from the proxy.
...
Set `DEFAULT_MODEL_PROVIDER=litellm` to route chat calls through the proxy:
...
The LiteLLM proxy supports every major LLM provider. Add the models you want to use to your LiteLLM configuration file, then set the corresponding API key environment variable.
