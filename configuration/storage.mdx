...
|---|---|
| `rustfs` | Serves S3 on port `59000` and a web console on `59001`. `RUSTFS_PORT` and `RUSTFS_CONSOLE_PORT` change them — if you change `RUSTFS_PORT`, change the port in `S3_ENDPOINT_URL` to match, or uploads cannot reach the store |
