...
| Secret location | `apps/admin` | Ragen Token Vault |
...
| Where tokens land | The `accounts` table | The vault, encrypted with AES-256-GCM |
...
<Steps>
  <Step title="Create an OAuth client in Google Cloud Console">
    ...
    No scopes need configuring on the client. Better Auth requests `openid`, `email`, and `profile` by default, and those three require no consent-screen review.
  </Step>
  <Step title="Set the credentials">
    ...
    Also confirm that `BETTER_AUTH_URL` matches the origin the panel is served on. If the panel is at `https://admin.example.com` but `BETTER_AUTH_URL` is `http://localhost:3200`, Google receives a localhost callback and fails with a redirect URI mismatch.
  </Step>
  <Step title="Set the allowed email domain">
    ...
    This gate applies only to account *creation*. Existing accounts are unaffected when you change it. The real access control is `User.role = 'admin'`, which is re-read from the database on every request.
  </Step>
  <Step title="Verify the configuration">
    ...
    | "That account is not a platform administrator" | OAuth worked, but the account doesn't have `User.role = 'admin'` — grant it from **Users** |
...
<Accordion title="redirect_uri_mismatch">
  ...
  - `BETTER_AUTH_URL` doesn't match the origin the browser is using
