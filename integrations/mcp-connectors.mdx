...
<Steps>
  ...
  <Step title="Confirm the connection">
    After authorising, the browser redirects back to Ragen. The connector's status changes to **Connected**. Your credentials are now stored in the Ragen Token Vault.
  </Step>
</Steps>

<Note>
  Ragen stores your OAuth tokens in the **Ragen Token Vault** — a separate service with AES-256-GCM encryption — not in the main application database. Your tokens are never written to Postgres.
</Note>
