...
Some private networks cannot be reached through RFC 1918 addresses. Railway's private network is an example: its `*.railway.internal` names resolve to an IPv6 unique-local address (`fd…`), and that may be the only address a service can reach, even where an IPv4 one is also published. Ragen refuses unique-local addresses even with the box ticked, because cloud metadata services live there too (`fd00:ec2::254` on EC2).
