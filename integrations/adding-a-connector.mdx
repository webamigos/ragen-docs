...
<Warning>
  A connector that carries a credential — `API_KEY_BEARER` or `EXTERNAL_MCP` —
  requires **HTTPS**, with no exception. A user's own API key or an OAuth token
  would otherwise travel in clear text. Plain `http://` is available only for
  `SERVER_SIDE` connectors, which carry no credential.
</Warning>
