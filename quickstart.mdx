...
<Steps>
  <Step title="Scaffold your installation">
    ...
    | Question | Default |
    |---|---|
    | Which LLM provider powers chat? | OpenRouter, Scaleway, OpenAI or Anthropic, then the key (Scaleway also asks for its project's base URL). It writes a matching route table. Anthropic has no embeddings endpoint, so on that key alone the knowledge base does not work, and the wizard tells you |
    | Where are uploaded documents stored? | **local disk**. S3 asks for bucket, region, endpoint and keys. **RustFS** starts an object store with the stack and generates its keys — see [Storage](/configuration/storage#bundled-object-storage-rustfs) |
