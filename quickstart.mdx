...
- **Docker and Docker Compose** — both must be running. The backing services (Postgres, Qdrant, LiteLLM, Temporal, Docling, Redis) all run in containers.
...
Once everything is running, use these curl commands to confirm each service is healthy:

```bash
# API server — should return {"status":"ok"}
curl http://localhost:3001/v1/healthcheck

# LiteLLM — lists the models it can actually reach
