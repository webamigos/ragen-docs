...
<AccordionGroup>
  ...
  <Accordion title="What are the minimum hardware requirements?">
    ...
    - **8 GB of RAM** to evaluate it, 16 GB for a small production install — mostly headroom. The containers a default `docker compose up` starts idle at about 900 MB together, document parsing (Docling) being most of it. [What the stack actually costs](/self-hosting#what-the-stack-actually-costs) breaks it down service by service and shows how to run a smaller stack.
    - **Docker** and Docker Compose
    - **Node.js `^24.15.0 || >=26.0.0`** if you run the apps outside containers — a range, not a floor
...
<AccordionGroup>
  ...
  <Accordion title="What data leaves my network?">
    ...
    - **Content moderation goes directly outward.** Moderation is a [guardrail rule](/security/guardrails) called `content-moderation`, switched on in the admin panel rather than by an environment variable. When it is on it calls OpenAI's moderation endpoint directly — the one model call the route table cannot redirect. Leave the rule off on an isolated install.

    See [Security and Privacy](/security/overview) for the full breakdown.
  </Accordion>

  <Accordion title="Is conversation data encrypted?">
    Encryption at rest is **opt-in in development and mandatory outside it**. Without a key provider configured, a development process starts and stores message content unencrypted, which keeps a fresh clone simple. Anywhere else — `NODE_ENV` other than `development`/`test`, and `TARGET_ENV` either unset or naming a real environment — a missing provider stops the process rather than writing plaintext, unless `ALLOW_UNENCRYPTED=1` waives it as a logged, deliberate choice. See [Encryption](/security/encryption).
