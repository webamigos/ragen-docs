...
| Redis | Background jobs — it is the queue. Required by the worker, and by the web app and API, which enqueue into it |
...
Building four images on every install is work nobody needs to repeat. Each
...
<Note>
  **These five serve the default job runtime, and only that.** All of them are
  BullMQ producers or consumers, which is what `WORKER_RUNTIME` defaults to. A
  deployment that sets `WORKER_RUNTIME=temporal` cannot use `ragen-worker`,
  `ragen-web` or `ragen-api` as published — the worker exits at boot and the
  two producers throw on the first upload. See
