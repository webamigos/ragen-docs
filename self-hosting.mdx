...
<Steps>
  <Step title="Start the backing services">
    ...
    This starts Postgres, Qdrant, Temporal, LiteLLM, Docling, and Redis in Docker. The applications themselves run on the host with hot reload.
    ...
    <Note>
      ...
      ```bash
      npm run ragen:up:app    # Postgres, Qdrant, and LiteLLM only
      ```
...
| LiteLLM | Every model call: chat, embeddings, and reranking — unless `LLM_GATEWAY=native`, which needs no proxy ([Model gateway](/configuration/model-gateway)) |
...
| Flag | Effect |
|------|--------|
| `FEATURE_FLAG_PII_MASKING` | A kill switch, not a switch: masking follows `PRESIDIO_ANALYZER_URL` and `PRESIDIO_ANONYMIZER_URL`, and `0` turns it off. The two containers use ~1 GB of RAM together and sit behind `--profile pii`, which is why it is opt-in. See [PII masking](/security/pii-masking). |
...
<Note>
  `LLM_GATEWAY=litellm` is here on purpose. Ragen calls providers directly by default, using a [route table](/configuration/model-gateway) that names Azure, Bedrock, Vertex and Scaleway — providers a new installation has no credentials for. This block starts a LiteLLM proxy instead, so it names that path explicitly rather than inheriting the default. Drop the line once you have provider credentials of your own.
</Note>
