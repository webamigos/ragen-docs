...
<Steps>
  <Step title="Start the backing services">
    ...
    <Note>
      If you only want to query existing knowledge bases and do not need document ingestion, use the lighter stack instead — it omits Redis, so there is no queue for the worker to drain:
