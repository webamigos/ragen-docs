...
<Steps>
  ...
  <Step title="Seed the database">
    Write the internal subscription plans that every organization is created against:

    ```bash
    npm run db:seed
    ```

    This is not optional — without it, organization creation fails. It reads configuration from `.env.local`.
  </Step>
...
| LiteLLM | Every model call: chat, embeddings, and reranking |
...
| Flag | Effect |
|------|--------|
| `FEATURE_FLAG_PII_MASKING` | Detect and mask personal data via Presidio before it reaches the model. Requires two extra containers (Presidio analyzer and anonymizer), which together use ~1 GB of RAM — that is why it is opt-in. |
...
Ragen's architecture is designed to support fully air-gapped deployments: the model layer is decoupled behind LiteLLM, and document parsing is local by default. Two honest caveats apply before you commit to this path.
...
Verify encryption is active by checking that new threads have `encryptedDek` populated in your database after the change.
