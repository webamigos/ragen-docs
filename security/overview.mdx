...
Every model call routes through a LiteLLM proxy that you also run. What leaves your network depends entirely on what that proxy points at:

| Configuration | What leaves your network |
|---|---|
| LiteLLM → locally-served model | Nothing, in normal operation |
| LiteLLM → commercial API | The prompt: the question plus the retrieved document chunks needed to answer it |
...
The application reports nothing to the vendor. There is no tag manager, no product analytics, and no usage ping in the Ragen app. An architecture test fails the build if a tag manager, analytics loader, or hardcoded measurement ID is added.
...
**Tenant isolation** is enforced separately. Each organisation gets its own Qdrant collection, and a guard logs any query that touches a tenant-scoped model without its organisation filter applied.
...
Personal data detection via Microsoft Presidio (including Polish-language recognisers for PESEL, NIP, REGON, ID card numbers, and IBAN) is available but **off by default**. Enable it with `FEATURE_FLAG_PII_MASKING=1`. Presidio runs as two additional containers, and per-organisation and per-file strictness settings are configured separately.
