...
Credentials are the exception to "all data lives in Ragen's database": connector OAuth tokens and the secret half of every API key are held by the [token vault](/configuration/token-vault), a separate service, encrypted with AES-256-GCM. Ragen's own database keeps only metadata and a masked value, so a database dump exposes no usable credential.
