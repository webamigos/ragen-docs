...
| **Features** | Turn features on or off for one organisation or as platform defaults; see which layer (org override / plan / platform default / built-in) is resolving each one — see [Organization features](#organization-features) |
| **Limits & Models** | Set storage ceilings, monthly token and cost caps, member caps, and model allowlists — per organisation and as platform defaults |
| **RAG Settings** | Set the retrieval pipeline's per-organisation switches — multi-query expansion, document summaries, reranking — and the platform defaults for them. The matching page inside the main app (**Organization → RAG settings**) shows these read-only |
...
On the **Features** page, a feature is on or off for an organization. Each one is decided by the first of these four layers that sets it:

1. an override for that organization;
2. the organization's plan;
3. the platform default;
4. the built-in default.

To change one organization, pick it at the top of the page, set the feature to **Force on** or **Force off**, and save. To change every organization where neither an override nor its plan sets the feature, set it under **Platform defaults**.

| Feature | Built-in default | What turning it off (or on) does |
|---|---|---|
| **Invite members** | off | On, owners and admins can invite people into the organization |
| **Public chatbot** | off | On, the organization can use the embedded widget and the hosted public assistant page. Off also stops serving the ones that already exist |
| **API access** | on | Off, the organization cannot create new API keys. Keys it already has keep working until you deactivate them under **API Keys** |
| **MCP connectors** | on | Off, the organization cannot add or connect connectors |
| **Custom assistant templates** | on | Off, the organization cannot create an assistant from a template |
| **Voice dictation** | off | On, the composer shows a microphone. Off, transcription requests are refused |
| **Public thread links** | off | On, people can share a thread by link. Off also stops links already shared from opening |
| **Add and remove documents** | on | Off, the knowledge base is frozen: nobody can upload or delete documents. Chat keeps working |
| **Create and delete projects** | on | Off, projects are frozen. Chat keeps working |
| **Change organization settings** | on | Off, the organization's settings are read-only |
| **Delete threads** | on | Off, nobody can delete a conversation in the panel: **Delete** disappears from the thread menu. Chat keeps working. This is meant for a showcase organization whose example threads every visitor should see. The public API's `DELETE /v1/threads/{id}` does not check this setting yet |
| **RAG readiness score (one LLM call per ingest)** | on | Off, uploads and re-processing skip the [readiness score](/concepts/knowledge-base#rag-readiness-score) call, **Score for RAG** disappears from the file menu, and no score is shown anywhere, including scores stored earlier. **Optimize** keeps working without a score |
| **Ragen Brain** | off | On, owners and admins can open [Ragen Brain](/concepts/brain) |
| **Ragen Brain: curate (extract, review, publish)** | on | Off, Brain is read-only for everyone, owners and admins included — no extraction, review, publishing, uploads into Brain or bundle export |
| **Ragen Brain: members may browse (read-only, sees every page)** | off | On, every member can browse Brain, read-only. **Read [what this discloses](/concepts/brain#letting-members-browse) before you turn it on** |
| **Ragen Brain: the operator's assistant** | off | On, Brain shows the [operator's assistant](/concepts/brain#the-operators-assistant). It has no effect unless **Ragen Brain** is on too |

The **Add and remove documents**, **Create and delete projects**, **Change organization settings** and **Delete threads** features exist so you can freeze an organization's content and configuration while its members can still read and ask questions. A showcase organization is the typical case.
