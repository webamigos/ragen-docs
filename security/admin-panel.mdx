...
Access to the panel is controlled by `User.role = 'admin'` — the platform administrator role. This is different from being an organisation owner or admin inside the main app. The dashboard layout re-reads this role from the database on every request, so revoking it takes effect immediately rather than waiting for a session to expire.

| Surface | Who reaches it | Scope |
|---|---|---|
| Main app (`:3000`) | Organisation owners and admins | Their own organisation only |
| Admin panel (`:3200`) | Users with `User.role = 'admin'` | The entire installation |
...
| **Proxy** | View LiteLLM proxy status — healthy and unhealthy deployments, which models are actually available, and per-organisation spend vs. budget |
...
|---|---|---|
| **Deactivate** | Clears `isActive`; the key stops authenticating immediately | Yes — the secret is preserved |
| **Revoke** | Deactivates the key, deletes the secret from the Token Vault, then deletes the row | No |

<Warning>
  Revoking a key requires `RAGEN_TOKEN_VAULT_URL` and `RAGEN_TOKEN_VAULT_SERVICE_SECRET` to be configured on the admin panel. Without them, the panel still lists and deactivates keys but shows a warning explaining why **Revoke** is unavailable. If the vault is unreachable mid-revoke, the key is left deactivated with its row intact — the panel tells you the secret survived rather than deleting the only record of it.
</Warning>
...
This is enforced, not merely absent. The Better Auth `admin` plugin ships an impersonate endpoint at `POST /api/auth/admin/impersonate-user`. Rather than leaving it accessible while removing the UI button, Ragen defines the platform admin role without the `user: ["impersonate"]` permission — the route itself rejects requests from platform admins.
