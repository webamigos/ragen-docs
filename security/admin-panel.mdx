...
To change one organization, pick it at the top of the page, set the feature to **Force on** or **Force off**, and save. To change every organization that has no override, set it under **Platform defaults**.
...
| **Delete threads** | on | Off, nobody in the organization can delete a conversation: **Delete** disappears from the thread menu. Chat keeps working. This is meant for a showcase organization whose example threads every visitor should see. The public API's `DELETE /v1/threads/{id}` does not check this setting yet |
