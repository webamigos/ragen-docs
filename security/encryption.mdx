...
<Warning>
  Encryption is **off by default in local development only**. With no key provider configured, a development process starts and stores message content in plaintext — intentionally, so a fresh clone runs without a cloud account. A deployed one behaves differently: outside `NODE_ENV=development` a process with no provider refuses to start rather than writing plaintext, unless `ALLOW_UNENCRYPTED=1` waives it deliberately. Verify that new threads are being encrypted with the SQL query in [Verifying encryption is active](#verifying-encryption-is-active) below.
</Warning>
