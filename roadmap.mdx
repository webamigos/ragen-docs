...
- **The hosted parser is a data-processing decision, not a performance one.** With `DOCUMENT_PARSER=mistral`, every ingested document is transmitted to a third party. That is the right trade for some deployments and disqualifying for others, which is why it will never be a silent fallback. See the [Security](/security) page.
