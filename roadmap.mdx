...
<Note>
  Nothing is being taken away. Qdrant stays the default vector store and Docling stays the default parser, and Temporal remains a supported runtime for anyone who selects it — in [`ragen-enterprise`](https://github.com/webamigos/ragen-enterprise), Apache-2.0 like Ragen itself, with its own worker image and its own parity suite.
</Note>

<Warning>
  **One upgrade is not a no-op, and it is the one that looks like one.** Temporal used to be what `WORKER_RUNTIME` resolved to when it was unset, so a deployment installed before the seam existed is running Temporal without ever having named it. That default is now `bullmq`. Upgrading such a deployment without setting anything moves it to a different engine at restart, and work already accepted by Temporal is orphaned — the new engine has never heard of it, and those documents sit in processing until something re-indexes them.

  Set `WORKER_RUNTIME=temporal` explicitly **before** you upgrade, and then decide about the move separately. If you do want BullMQ, follow [switching runtimes on a running deployment](/configuration/background-jobs#switching-runtimes-on-a-running-deployment): drain first, delete the schedules on the engine you are leaving, then switch.

  A deployment that already sets `WORKER_RUNTIME` explicitly, either way, is unaffected.
</Warning>
